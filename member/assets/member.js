...
async function login(no,code){const auth=await transact({action:'member_auth_start',environment:ENV,member_no:no,code:code,client_id:'member-portal-v2',request_id:requestId()});if(!auth||!auth.ok||!auth.member_session)throw new Error(auth&&auth.error?auth.error:'Нэвтрэх мэдээлэл буруу байна');const snap=auth.snapshot&&auth.snapshot.ok?auth.snapshot:null;const item={memberNo:String(no),session:auth.member_session,environment:ENV,member:snap&&snap.member?snap.member:null};saveSession(item);return snap||read()}
